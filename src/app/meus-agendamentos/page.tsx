...
export default async function MeusAgendamentos() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 font-sans text-zinc-100 pb-20">
      ...
      {/* NAVBAR RESPONSIVA */}
      <nav className="max-w-6xl mx-auto px-6 py-5 flex justify-between items-center border-b border-zinc-900 mb-10">
        ...
        {/* MOBILE — só dois botões compactos */}
        <div className="flex sm:hidden items-center gap-2">
          <Link href="/agendar" className="bg-yellow-500 hover:bg-yellow-400 text-zinc-950 px-3 py-2 rounded-lg font-bold transition-colors text-xs">
            + Agendar
          </Link>
          <LogoutButton variant="outline" />
        </div>
      ...
      <main className="max-w-6xl mx-auto px-6">
        <header className="mb-10 flex items-center justify-between">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-white mb-1">Olá, {firstName}</h1>
            <p className="text-zinc-500 text-sm">Gerencie seus horários agendados</p>
          </div>
          <Link href="/" className="text-zinc-500 hover:text-white text-sm transition-colors hidden sm:block">
            ← Início
          </Link>
        </header>
        ...
        <section className="mb-16">
          <h2 className="text-lg font-bold text-white mb-6">Próximos Agendamentos</h2>
          {upcoming.length === 0 ? (
            <div className="p-8 border border-dashed border-zinc-800 rounded-2xl text-center text-zinc-500">
              ...
              <Link href="/agendar" className="text-yellow-500 hover:underline">Agendar agora</Link>
            </div>
          ) : (
                ...
                <div key={app.id} className="bg-[#111111] border border-zinc-800 rounded-2xl p-6 flex flex-col justify-between hover:border-zinc-700 transition-colors">
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="border border-yellow-500/30 text-yellow-500 bg-yellow-500/10 px-3 py-1 rounded-full text-xs font-semibold flex-shrink-0">
                        Confirmado
                      </span>
                    </div>
        ...
        {/* HISTÓRICO */}
        <section>
          <h2 className="text-lg font-bold text-white mb-6">Histórico</h2>
          {past.length === 0 ? (
            <div className="p-8 border border-dashed border-zinc-800 rounded-2xl text-center text-zinc-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {past.map(app => {
                const alreadyReviewed = app.reviews && app.reviews.length > 0;
                const canReview = app.status === 'COMPLETED' && !alreadyReviewed;
                return (
                  <div key={app.id} className="bg-[#0a0a0a] border border-zinc-800/50 rounded-2xl p-6 hover:border-zinc-700 transition-colors">
                    <div className="flex justify-between items-start mb-1">
                      <h3 className="text-lg font-bold text-white">{app.barber.user.name}</h3>
                    ...
                    <div className="pt-4 border-t border-zinc-800/50 flex justify-between items-center">
                      ...
                      {alreadyReviewed && (
                        <div className="flex items-center gap-0.5">
                          {[1,2,3,4,5].map(s => (
                            <svg key={s} className={`w-3.5 h-3.5 ${s <= app.reviews[0].rating ? 'text-yellow-400' : 'text-zinc-700'}`} fill="currentColor" viewBox="0 0 24 24">
                    ...
                    {canReview && (
                      <ReviewWrapper appointmentId={app.id} barberName={app.barber.user.name} />
                    )}
                  </div>
                );
              })}
